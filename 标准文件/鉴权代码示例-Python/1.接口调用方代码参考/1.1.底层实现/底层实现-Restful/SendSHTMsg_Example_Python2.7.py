...
#调用石化通消息推送服务，发送石化通群机器人消息示例 适配Python2.7 使用pycryto库实现AES CBC加密
...
# URL编码
def urlencode(s):  
    return urllib.quote(s.encode('utf-8'))  # python2.7中使用quote
# URL编码
...
def GetSignatureSha256(data, key):  
    key = bytes(key)  # 将密钥转换为字节串  
    data = bytes(data)  # 将数据转换为字节串  
...
if x.status_code == 200:
    ...
    f.close()
# 将发送的信息写入本地日志/var/log/example/shtsendmsg.log中